...
let categoryList = [];
let colorsCategory = ['#8AA4FF', '#FF0000', '#2AD300', '#FF8A00', '#E200BE', '#0038FF'];
...
function createdNewCategory() {
    ...
    for (let color = 0; color < colorsCategory.length; color++) {
        ...
        document.getElementById('colorsContainer').innerHTML += newCategoryColors(colorCategory);
    }
};
...
async function createNewCategoryTask() {
    ...
    if (newCategory) {
        categoryList.push({ name: newCategory, status: false, color: newCategoryColor });
...
function renderCategory(categories) {
    ...
    categories.forEach((category, index) => {
        ...
        const categoryName = (category && category.name) ? category.name : 'Unnamed Category';
        const categoryColor = (category && category.color) ? category.color: 'Unnamed Color';
